...
#define OP_HANDLER(_name) void M6800::_name()
...
OP_HANDLER( cli )
{
	const bool was_masked = (CC & M6800::kFlagInterruptMask) != 0U;
	CLI;
	if (was_masked)
		defer_irq_check();
}
...
OP_HANDLER( cmpx_im )
{
	...
	CLR_NZV;
	SET_Z16(r);
	uint8_t x = high_byte(d) - high_byte(b);
	SET_N8(x);
	CC |= static_cast<uint8_t>((
		((high_byte(d) & 0x80U) != 0U && (high_byte(b) & 0x80U) == 0U && (x & 0x80U) == 0U) ||
		((high_byte(d) & 0x80U) == 0U && (high_byte(b) & 0x80U) != 0U && (x & 0x80U) != 0U)) ? 2U : 0U);
}
