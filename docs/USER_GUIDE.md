...
JR-200 Web Emulatorをブラウザで使うためのガイドです。画面の番号（01〜09）に沿って、各機能の目的、操作、結果を説明します。[実際の起動画面](../README.md#起動画面と操作の流れ)、ビルドとROMの用意はREADMEを参照してください。
...
macOSの日本語入力が`Ctrl+3`を顔文字入力として先に処理する場合、Webページにはキー操作が届かないため抑止できません。その場合はmacOSの入力ソースを英数に切り替えるか、仮想キーボードでCTRLを押してから3を押してください。
...
Wikiなどの作品別 `?game=<id>` リンクは、公開カタログに登録されたCJRを検証して通常の
カセット入力へ自動マウントします。`?game=<id>`だけでは自動実行しないので、
ROM・フォントで起動して`MLOAD`と作品ごとの実行コマンドを入力してください。
`&launch=1`を付けた場合は、対応するROMと作品版に限り、通常のカセット読込みと
内容の検証後に起動を支援します。ROMやフォントは取得しません。
現時点で公開カタログは空のため、公開Pagesで作品リンクはまだ使えません。
[作品IDリンクの配布条件](GAME_LINKS.md)も参照してください。

手元のCJRと入力手順を一緒に渡された場合は、04の「起動パックZIP」または
「起動パックのフォルダー」から取り込めます。任意の手順は入力支援欄へ置くだけで
自動入力しません。条件を満たすパックだけ通常カセット経路の起動支援に進みます。
詳しくは[起動パック](LAUNCH_PACK.md)を参照してください。
...
| 画面倍率・画素比・回転・スムージング | 自動倍率は表示領域に収まる最大サイズへ縦横比を保って追従。1〜5倍の固定倍率も選べる。通常は正方形画素・0度・補間なし |
