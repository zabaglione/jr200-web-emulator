...
対象: https://github.com/zabaglione/jr200-web-emulator （private）
...
ソース追加・削除時は `python3 scripts/update_manifest.py` と `make check`。ROM、メーカー由来フォント、録音、商用テープ、秘密情報はコミットしない。privateを維持し、Pagesや公開サイトへの自動デプロイは行わない。
