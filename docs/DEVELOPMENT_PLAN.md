...
| P05 | [MN1271・MN1544・CRTCとシステム時刻](issues/P05.md) | P04 | 未実装。画面が出るだけで周辺回路の完了としない。 |
| P06 | [ブラウザBASIC起動・ROM選択・入力と画面](issues/P06.md) | P05 | 未実装。現在のWebページはCJR検査ツールのみ。 |
| P07 | [開発用デバッガ・追跡・メモリウォッチ](issues/P07.md) | P06 | 未実装。JR-800等での利用方針を反映した検証機能。 |
| P08 | [CJRカセットLOAD/MLOAD/SAVE/MSAVE統合](issues/P08.md) | P07 | 未実装。CJRファイルの検査成功と通常カセット動作を区別する。 |
| P09 | [音声のWeb出力・休止復帰](issues/P09.md) | P08 | 未実装。音声デバイスの確認が必要。 |
| P10 | [CJR信号生成・WAVエンコード](issues/P10.md) | P09 | 未実装。GetLoadData/WaveGetterに公開根拠はある。実機成功はP12で判定。 |
...
P00〜P04は受入完了。次の依存工程はP05のMN1271・MN1544・CRTCとシステム時刻である。スクリプトによる自動closeはしない。実機試験に進めない場合も、未検証を隠して依存工程を完了扱いにしない。
