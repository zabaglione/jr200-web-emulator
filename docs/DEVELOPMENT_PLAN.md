...
| P11 | [録音WAV解析・CJR復元](issues/P11.md) | P10 | 2026-09-22ローカル実装・技術条件確認済み。Actions枠待ちで未push/remote受入保留。 |
...
P00〜P10は受入完了。P11はローカル技術条件を満たしたが、Actions枠reset後のpushと
remote CI、Issue closeまで受入保留である。P11を受け入れるまでP12へ進まない。
スクリプトによる自動closeはしない。実機試験に進めない場合も、未検証を隠して依存工程を
完了扱いにしない。
