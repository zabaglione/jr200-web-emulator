...
登録日: 2026-09-21。対象は `zabaglione/jr200-web-emulator`（private）。以下はGitHubから返された実際のIssue番号。P00〜P02は初期実装あり・受入レビュー待ち、P03以降は未着手。初期設定ではIssueをcloseしていない。
...
まず #1〜#3 の受入条件と既存証拠をレビューし、その後 #4 の独立対照試験へ進む。未合格の依存を無言で飛ばさない。初期ファイルの登録とエミュレータの完成は別である。
