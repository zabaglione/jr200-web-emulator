...
登録日: 2026-09-21。対象は `zabaglione/jr200-web-emulator`（private）。以下はGitHubから返された実際のIssue番号。2026-09-22までにP00〜P07をremoteで受け入れ、P08をローカルで受け入れた。
...
#1〜#8は順にremote受入完了。#9はローカル受入完了で、実装commitのCI成功後にcloseする。次は#10の音声Web出力・休止復帰である。未合格の依存を無言で飛ばさない。通常CJR transportと、実音・WAV変換・実機WAV往復の合格は別である。
