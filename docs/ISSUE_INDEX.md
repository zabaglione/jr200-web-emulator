...
登録日: 2026-09-21。対象は `zabaglione/jr200-web-emulator`（private）。以下はGitHubから返された実際のIssue番号。2026-09-22までにP00〜P06を順に受け入れた。
...
#1〜#7は順に受入完了。次は#8の開発用デバッガ・追跡・メモリウォッチへ進む。未合格の依存を無言で飛ばさない。ブラウザBASIC起動と、音声・通常カセット・実機WAV往復の合格は別である。
