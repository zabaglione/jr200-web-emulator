...
登録日: 2026-09-21。対象は `zabaglione/jr200-web-emulator`（private）。以下はGitHubから返された実際のIssue番号。2026-09-22までにP00〜P03を順に受け入れ、P04以降は未着手である。
...
#1〜#4は順に受入完了。次は#5のCPUコア・メモリバス移植へ進む。未合格の依存を無言で飛ばさない。初期ファイルの登録とエミュレータの完成は別である。
