...
## 10. 変更・完了管理
各Issueには依存関係、対象、対象外、成果物、テスト、受入条件を持たせる。課題は「実装済」「合成テスト済」「上流対照済」「実機試験済」を別に記録する。P13はP11に依存し、P12は初版後の独立検証としてopenのまま残す。未検証の条件を別Issueの完了で上書きせず、自動closeしない。期間・費用見積は、上流差分試験と初回ブートの難易度を観測してから別途更新する。
